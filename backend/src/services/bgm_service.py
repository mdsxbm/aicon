...
class BGMService(BaseService):
    ...
    async def upload_bgm(
        ...
        try:
            ...
            storage_client.upload_file(file_key, content)
    ...
    async def _extract_audio_duration(self, content: bytes, file_ext: str) -> Optional[int]:
        """
        使用ffprobe提取音频时长
    ...
    async def delete_bgm(self, bgm_id: str, user_id: str) -> bool:
        ...
        try:
            # 从MinIO删除文件
            if bgm.file_key:
                storage_client.delete_file(bgm.file_key)
